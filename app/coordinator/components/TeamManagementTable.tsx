...
import {
  updatePlayerAction,
  deletePlayerAction,
} from "@/lib/actions/players/player-management.action";
import { getPlayersBySchoolIdAction } from "@/lib/actions/players/get-palyer.action";
...
const TeamManagementTable = ({
  ...
  const [loadingType, setLoadingType] = useState<
    "captain" | "vice" | "delete" | "refresh" | null
  >(null);
  ...
  const handleRefresh = () => {
    ...
    startTransition(async () => {
      try {
        const result = await getPlayersBySchoolIdAction(schoolId);
        if (result.success) {
          setLocalPlayers(result.players);
        } else {
          setRefreshError(result.message || "Failed to refresh players");
        }
      } catch (error) {
        console.error("Error refreshing players:", error);
        setRefreshError("Unable to fetch latest data. Please try again.");
      } finally {
        setLoadingId(null);
        setLoadingType(null);
      }
  ...
  return (
    <div className="w-full lg:h-[80vh] flex flex-col rounded-lg shadow-sm mt-3 border border-gray-700 ">
      <div className="flex items-center justify-end gap-3 px-4 py-2 border-b border-gray-700">
        ...
        >
          {loadingType === "refresh" && <Spinner />}
          {loadingType === "refresh" ? "Refreshing" : "Revalidate"}
        </Button>
      </div>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                      ...
                      <DropdownMenuItem
                        ...
                        disabled={loadingId === item._id || isPending}
                      >
                        {loadingId === item._id && loadingType === "vice" && (
                          <Spinner />
                        )}
